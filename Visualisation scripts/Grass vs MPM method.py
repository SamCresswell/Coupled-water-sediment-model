...
df = pd.read_excel(filepath, sheet_name='Single_slope_Grass')
...
grass_cases = df[df['case'] == 'Grass']
MPM_cases = df[df['case'] == 'MPM']

unique_values_grass = grass_cases['Hyp'].unique().tolist()
unique_values_MPM = MPM_cases['Hyp'].unique().tolist()

for value in unique_values_grass:
    
    index = unique_values_grass.index(value)
    
    colour = all_colours[index]
    
    subset = grass_cases[grass_cases['Hyp'] == value]
    
    ax1.plot(subset['q'], subset['q_b'],linewidth=line_width, color = colour,zorder = 10, markersize=1, marker = 'x', markeredgewidth = .1, label = value)
    
for value in unique_values_MPM:
    
    index = unique_values_MPM.index(value)
    
    colour = all_colours[index]
    
    subset = MPM_cases[MPM_cases['Hyp'] == value]
    
    ax1.plot(subset['q'], subset['q_b'],linewidth=line_width, color = colour,zorder = 10, markersize=1, marker = 'x', markeredgewidth = .1, label = '_nolegend_',linestyle = 'dotted')
...
for value in unique_values_grass:
    ...
    ax2.plot(subset['q'], subset['u_b'],linewidth=line_width, color = colour,zorder = 10, markersize=1, marker = 'x', markeredgewidth = .1, label = '_nolegend_')
...
for value in unique_values_MPM:
    ...
    ax2.plot(subset['q'], subset['u_b'],linewidth=line_width, color = colour,zorder = 10, markersize=1, marker = 'x', markeredgewidth = .1, label = '_nolegend_',linestyle = 'dotted')
    

ax1.set_xlabel('Peak flowrate $q$ (m$^2$s$^-$$^1$)',fontsize=axistitlesize*scalefactor)
ax1.set_ylabel('Gravel deposition distance downstream from GST (m)',fontsize=axistitlesize*scalefactor)
ax2.set_xlabel('Specific flowrate $q$ (m$^2$s$^-$$^1$)',fontsize=axistitlesize*scalefactor)
ax2.set_ylabel('Bed velocity $u_b$ (m$^2$s$^-$$^1$)',fontsize=axistitlesize*scalefactor)
...
save_file_path = r"../Results/Graphical results/Grass_vs_MPM_equations.png"
